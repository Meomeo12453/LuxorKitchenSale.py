...
# parent_id là Ghi chú nếu tồn tại trong Mã khách hàng
def get_parent_id(x):
...
# Đệ quy tìm toàn bộ thuộc cấp của một node
...
# Tính override_comm đúng logic nhóm
network = {
...
# Sắp xếp để các dòng cùng parent_id liên tiếp nhau (cha trước con)
df_export = df.sort_values(
    by=['parent_id', 'Mã khách hàng'],
    ascending=[True, True],
    na_position='last'
)
